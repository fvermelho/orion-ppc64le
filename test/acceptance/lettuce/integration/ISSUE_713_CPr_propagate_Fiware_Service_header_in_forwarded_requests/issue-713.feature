...
#TODO: Check if there is more operations forwarded
  ...
  Scenario: Fiware-Service header is forwarded to a Context Provider query operation
    ...
    # First registration
    And a new "NGSI9" api request with the service "issue_713" and the subservice "/subservice"
    ...
    # Query consult
    And a new "NGSI10" api request with the service "issue_713" and the subservice "/subservice"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
