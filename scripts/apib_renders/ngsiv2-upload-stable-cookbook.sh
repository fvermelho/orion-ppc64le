...
#
# Mostly a verbatim copy of NGSIv2-Update-Cookbook-Spec job at jenkins
...
apiary preview --path ${WORKSPACE}/doc/apiary/v2/fiware-ngsiv2-cookbook-rc-2016_05_31.apib --output /tmp/tmp.html

# Change now to gh-pages branch in fiware-orion repo
git checkout gh-pages

# Sync gh-pages in fiware-orion repository
COUNT=$(diff /tmp/tmp.html api/v2/stable/cookbook/index.html | wc -l)
if [ $COUNT -ne 0 ]
then
  cp /tmp/tmp.html api/v2/stable/cookbook/index.html
  git add api/v2/stable/cookbook/index.html
  git commit -m "ADD ngsi v2 (stable) apiary cookbook snapshoot: $BUILD_NUMBER" api/v2/stable/cookbook/index.html
  git push origin gh-pages
fi

# Sync gh-pages in FIWARE specifications repository
COUNT=$(diff /tmp/tmp.html /tmp/specifications/ngsiv2/stable/cookbook/index.html | wc -l)
if [ $COUNT -ne 0 ]
then
  cp /tmp/tmp.html /tmp/specifications/ngsiv2/stable/cookbook/index.html
  cd /tmp/specifications && git commit -m "ADD ngsi (stable) v2 apiary cookbook snapshoot: $BUILD_NUMBER" ngsiv2/stable/cookbook/index.html
